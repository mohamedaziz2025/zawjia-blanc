...
function AdminStatCard({ icon: Icon, label, value, accent, trend }: {
  ...
  return (
    <motion.div variants={fadeUp}
      ...
      <p className="font-display font-bold" style={{ color:'#E8E3D5', fontSize:'1.6rem', letterSpacing:'-0.04em' }}>
        {typeof value === 'number' ? value.toLocaleString() : value}
      </p>
      <p className="text-xs mt-0.5" style={{ color:'rgba(232,227,213,0.4)' }}>{label}</p>
    </motion.div>
  );
}
...
export default function AdminDashboard() {
  ...
  const chartData = [
    ...
    { name:'Membres',  value:stats?.totalUsers ?? 0,          fill:'rgba(255,255,255,0.15)' },
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color:'rgba(232,227,213,0.4)' }}>Vue d&apos;ensemble de la plateforme Zawjia</p>
        </div>
      ...
      <motion.div initial="hidden" animate="show" variants={stagger} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <AdminStatCard icon={Brain}  label="Logs IA"              value={stats?.totalAiLogs ?? 0}        accent="rgba(232,227,213,0.5)"/>
      </motion.div>
        ...
        <motion.div initial={{ opacity:0,y:18 }} animate={{ opacity:1,y:0 }} transition={{ delay:0.2,duration:0.5,ease:[0.16,1,0.3,1] }}
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            {[
              ...
              { label:'Walis enregistrés', value:stats?.totalWalis ?? 0, accent:'rgba(232,227,213,0.5)' },
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={label}>
                <div className="flex justify-between text-[11px] mb-1" style={{ color:'rgba(232,227,213,0.4)' }}>
                  <span>{label}</span>
        ...
        <motion.div initial={{ opacity:0,y:18 }} animate={{ opacity:1,y:0 }} transition={{ delay:0.3,duration:0.5,ease:[0.16,1,0.3,1] }}
          ...
          <ResponsiveContainer width="100%" height={180}>
            <RadialBarChart cx="50%" cy="50%" innerRadius="30%" outerRadius="90%" data={chartData}>
              ...
              <Tooltip contentStyle={{ background:'#0d1117', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8, fontSize:12, color:'#E8E3D5' }}
                formatter={(v:number,n:string) => [v.toLocaleString(), n]}/>
            </RadialBarChart>
          </ResponsiveContainer>
          <div className="space-y-2 w-full mt-1">
            {chartData.map(d => (
              <div key={d.name} className="flex items-center gap-2 text-xs" style={{ color:'rgba(232,227,213,0.5)' }}>
                <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background:d.fill }}/>
                <span>{d.name}</span>
                <span className="ml-auto font-bold" style={{ color:'#E8E3D5' }}>{d.value.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </motion.div>
