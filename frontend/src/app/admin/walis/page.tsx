...
export default function AdminWalisPage() {
  ...
  const LABEL_STYLE = { color:'#9ca3af', fontSize:'11px', marginBottom:'2px', display:'block' };
  const VAL_STYLE   = { color:'#111827', fontSize:'13px' };
                  ...
                  <tbody>
                    {verified.map((w, i) => (
                      <motion.tr key={w._id}
                        ...
                        <td className="px-4 py-3 font-medium" style={{ color:'#111827' }}>{w.name}</td>
                        <td className="px-4 py-3" style={{ color:'#6b7280' }}>{relationshipLabel(w.relationship)}</td>
                        <td className="px-4 py-3 text-xs" style={{ color:'#6b7280' }}>{w.phone ?? w.email ?? '—'}</td>
                        ...
                        <td className="px-4 py-3 text-xs" style={{ color:'#9ca3af' }}>{formatDate(w.createdAt)}</td>
                      </motion.tr>
                    ))}
                  </tbody>
                </table>
