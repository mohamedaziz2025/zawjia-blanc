...
const STATUS_MAP: Record<string, { label: string; color: string; bg: string; border: string }> = {
  ...
  rejected: { label: 'Non retenu',       color: 'rgba(232,227,213,0.35)', bg: 'rgba(255,255,255,0.04)', border: 'rgba(255,255,255,0.08)' },
};
...
function MatchCard({ match }: { match: Match }) {
  ...
  return (
    <motion.div variants={fadeUp}
      style={{ background: 'rgba(17,22,32,0.85)', border: '1px solid rgba(255,255,255,0.07)', boxShadow: '0 2px 24px rgba(0,0,0,0.3)' }}
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-sm" style={{ color: '#E8E3D5' }}>Profil anonyme</p>
            <p className="text-[11px]" style={{ color: 'rgba(232,227,213,0.35)' }}>{formatDate(match.createdAt)}</p>
          </div>
        </div>
      ...
      {/* Details grid */}
      <div className="grid grid-cols-2 gap-2.5 text-[11px]" style={{ color: 'rgba(232,227,213,0.5)' }}>
        {[
          { icon: <span className="w-1.5 h-1.5 rounded-full inline-block" style={{ background: match.user1Choice ? '#2D7D52':'rgba(255,255,255,0.15)' }}/>, label: `Frère : ${match.user1Choice ? 'Intéressé' : 'En attente'}` },
          { icon: <span className="w-1.5 h-1.5 rounded-full inline-block" style={{ background: match.user2Choice ? '#2D7D52':'rgba(255,255,255,0.15)' }}/>, label: `Sœur : ${match.user2Choice ? 'Intéressée' : 'En attente'}` },
        ...
        <div className="space-y-2 pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          {waliContact && (
            <div className="p-3.5 rounded-xl text-xs space-y-1.5"
              ...
              <p style={{ color: 'rgba(232,227,213,0.7)' }}>{waliContact.name} — {waliContact.relationship}</p>
              <p style={{ color: 'rgba(232,227,213,0.55)' }}>{waliContact.phone}</p>
              <p style={{ color: 'rgba(232,227,213,0.4)' }}>{waliContact.email}</p>
            </div>
          )}
        </div>
      )}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: 'rgba(232,227,213,0.4)' }}>Gérez vos connexions et prises de contact</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Non retenus', count: groups.rejected.length,  accent: 'rgba(232,227,213,0.3)' },
        ].map(({ label, count, accent }) => (
          <div key={label} className="rounded-2xl p-4 text-center"
               style={{ background: 'rgba(17,22,32,0.8)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <p className="font-display font-bold text-2xl" style={{ color: accent, letterSpacing: '-0.04em' }}>{count}</p>
            <p className="text-[11px] mt-0.5" style={{ color: 'rgba(232,227,213,0.4)' }}>{label}</p>
          </div>
        ))}
      </div>
      ...
      {isLoading ? (
        <div className="flex justify-center h-48 items-center gap-3 flex-col">
          ...
          <p className="text-sm" style={{ color: 'rgba(232,227,213,0.35)' }}>Chargement…</p>
        </div>
      ) : matches.length === 0 ? (
        <div className="h-[50vh] flex flex-col items-center justify-center gap-4 text-center">
          ...
          <h3 className="font-display font-semibold" style={{ color: '#E8E3D5', fontSize: '1.05rem', letterSpacing: '-0.02em' }}>
